...
- 将第二个随机值写入 `AUTH_SECRET_KEY`，并将它作为需要长期备份的生产密钥保存。后端会使用带独立用途标识的 HMAC-SHA256，从它分别派生登录认证所需密钥之外的赛事账号加密子密钥和去重子密钥。
- 首次导入赛事账号后不得直接修改 `AUTH_SECRET_KEY`，否则已有账号凭据将无法解密。未来确需轮换时，必须先实现密钥版本和数据重加密流程。
