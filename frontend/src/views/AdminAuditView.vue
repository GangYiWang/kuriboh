...
<template>
  <div class="page-shell admin-page">
    ...
    <div v-if="data?.items.length" class="audit-list">
      <article v-for="item in data.items" :key="item.id" class="audit-row"><time>{{ formatTime(item.created_at) }}</time><div><strong>{{ item.action_type }}</strong><p>{{ item.operator_nickname }} · {{ item.target_type }} / {{ item.target_id }}</p></div><details><summary>数据变化</summary><pre>{{ JSON.stringify({ before: item.before_json, after: item.after_json }, null, 2) }}</pre></details></article>
    </div>
