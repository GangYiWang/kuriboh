...
示例配置只用于本地开发。部署前必须修改数据库密码和 `AUTH_SECRET_KEY`，并确保 `.env` 不进入版本控制。`AUTH_SECRET_KEY` 同时是赛事账号加密子密钥的派生根，首次导入账号后必须保持不变并安全备份。
...
- `GET /api/admin/health`：平台管理员权限边界检查，仅 `PLATFORM_ADMIN` 可访问。
- 所有错误统一返回 `code`、`message`、`details` 和 `request_id`。
- 响应通过 `X-Request-ID` 返回请求追踪标识，日志使用结构化 JSON。

全局角色仅包含 `USER` 与 `PLATFORM_ADMIN`。赛事管理员和选手是具体赛事中的身份：创建者通过 `created_by_id` 获得该赛事管理权限，报名或进入正式名单后成为选手；两种身份可以同时存在。`PLATFORM_ADMIN` 只控制禁卡表、平台公告、平台通知和全局审计等平台级能力，不自动获得其他用户赛事的管理权限。密码使用 Argon2 哈希，登录凭证使用带有效期的 JWT；平台权限与赛事所有权均由后端依赖统一校验。

本地账号可选择中国大陆手机号或 QQ 号注册，登录时两种标识使用同一输入框。手机号与 QQ 号跨字段保持唯一，既有 QQ 账号继续有效；QQ OAuth 的 OpenID 绑定仍与本地登录标识分开保存。

## 创建初始平台管理员

先完成迁移，再使用交互式命令创建平台管理员，避免把密码留在终端历史中：
...
图片上传接受有效的 JPG、JPEG、PNG 和 WEBP，单文件上限 20MB、最大尺寸 6000×6000；iPhone HDR/MPO 照片会提取第一帧并转码为标准 JPEG，服务会验证并重新编码图片，文件名随机生成。
...
赛事草稿不对选手公开；发布后立即开放报名，不设置报名起止时间。任何登录用户都可以创建赛事并成为该赛事的赛事管理员，也可以报名自己或其他用户创建的赛事，但不能绕过报名审核和容量限制。审核通过和恢复操作使用赛事行锁串行检查容量，避免并发超卖。存在待审核报名时禁止开赛；开赛会关闭报名、生成正式参赛名单快照并锁定容量、瑞士轮轮数、Top N 和禁卡表版本，但不会自动生成第 1 轮。

瑞士轮由管理员生成预览、调整并正式发布。第一轮随机配对，后续轮优先避免重复对手，其次尽量同胜场配对；奇数人数自动产生 BYE。双方独立提交胜/负，一致时自动确认，胜/胜或负/负进入冲突并由管理员裁定；管理员也可在瑞士轮将未参赛的双方裁定为各负一场，淘汰赛不允许双败。排名固定使用胜场、OMW、败局轮次小分、直接交手和昵称稳定排序；发布下一轮才锁定上一轮赛果。管理员只能在当前轮结束且下一轮未发布时执行退赛，未发布预览会自动作废。
...
- 生产环境必须替换 `AUTH_SECRET_KEY` 和开发数据库密码。后端使用带独立用途标识的 HMAC-SHA256，从 `AUTH_SECRET_KEY` 分别派生赛事账号加密子密钥和去重子密钥，并不会直接拿认证签名密钥加密账号。赛事账号写入后不得随意更换 `AUTH_SECRET_KEY`，否则已有账号将无法读取；如需轮换，必须先实现密钥版本和数据重加密流程。
