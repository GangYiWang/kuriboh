...
赛事草稿不对选手公开；发布后立即开放报名，不设置报名起止时间。普通用户和赛事管理员均可报名，赛事管理员不会因拥有后台权限而被排除在参赛名单之外，也不能绕过报名审核和容量限制。审核通过和恢复操作使用赛事行锁串行检查容量，避免并发超卖。存在待审核报名时禁止开赛；开赛会关闭报名、生成正式参赛名单快照并锁定容量、瑞士轮轮数、Top N 和禁卡表版本，但不会自动生成第 1 轮。
